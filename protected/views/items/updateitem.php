...
<form action="" method="POST" id="addProdForm">
	...
	<div class="content-white">
		...
		<li class="roomsCount">
			...
			<ul class="rooms-list">
			    <li><label class="radio <?php echo ($model->rooms == 1) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 1) ? "checked" : "" ?> name="rooms-number" value="1" />1</label></li>
			    <li><label class="radio <?php echo ($model->rooms == 2) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 2) ? "checked" : "" ?> name="rooms-number" value="2" />2</label></li>
			    <li><label class="radio <?php echo ($model->rooms == 3) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 3) ? "checked" : "" ?> name="rooms-number" value="3" />3</label></li>
			    <li><label class="radio <?php echo ($model->rooms == 4) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 4) ? "checked" : "" ?> name="rooms-number" value="4" />4</label></li>
			    <li><label class="radio <?php echo ($model->rooms == 5) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 5) ? "checked" : "" ?> name="rooms-number" value="5" />5</label></li>
			    <li><label class="radio <?php echo ($model->rooms == 40) ? "checked" : "" ?>"><input type="radio" <?php echo ($model->rooms == 40) ? "checked" : "" ?> name="rooms-number" value="40" />5+</label></li>
			</ul>
		    </div>
		</li>
		<li>
		    ...
		    <div class="right-part">
			<textarea id="titleReklText" class="small" name="addnew[slogan]" maxlength="90" placeholder="Опишите вашу недвижимость с выгодной стороны..."><?php echo $model->slogan; ?></textarea>
		...
		<li>
		    ...
		    <div class="right-part">
			<input id="city" type="text" class="rfield" name="addnew[city]" value="<?php echo $model->city; ?>" />
		    </div>
		</li>
		<li>
		    <span class="label">В районе<span class="mark">*</span>:</span>
		    <div class="right-part">
			<?php $rayon_model = Districts::model()->findByPk($model->district); ?>
			<input id="out_rayon" class="rfield" type="text" name="addnew[rayon]" value="<?php echo isset($rayon_model) ? $rayon_model->name : ""; ?>" />
			<input id="out_rayon_id" type="hidden" name="addnew[rayon_id]" value="<?php $model->district; ?>" />
		...
		<li>
		    ...
		    <?php $street_model = Streets::model()->findByPk($model->street); ?>
		    <div class="right-part">
			<input id="out_street" class="rfield" type="text" name="addnew[street]" value="<?php echo isset($street_model) ? $street_model->name : ''; ?>" />
			<input id="out_street_id" type="hidden" name="addnew[street_id]" value="<?php echo $model->street; ?>" />
		...
		<li>
		    ...
		    <div class="right-part">
			<label class="small-input"><input type="text" class="rfield" name="addnew[house]" value="<?php echo $model->house; ?>"/></label>
		    </div>
		    <div class="clear"></div>
		</li>
		<li>
		    ...
		    <div class="right-part">
			<label class="small-input"><input type="text" name="addnew[housing]" value="<?php echo $model->housing; ?>"/></label>
		    </div>
		</li>
	...
	<div class="content-white">
		...
		<?php echo $this->actionGetOneStepFields($model->functionality, $model->id); ?>
		<li class="firstField">
		    <span class="label">Площадь, м<sup>2</sup><span class="mark">*</span>:</span>
		    <div class="right-part">
			<label class="meter-input"><input id="area" name="addnew[area]" type="text" placeholder="0" value="<?php echo $model->area; ?>" /></label>
		    </div>
		</li>
		<li>
		    <span class="label">Стоимость, руб.<span class="mark">*</span>:</span>
		    <div class="right-part">
			<label class="price-input with-details"><input id="price" name="addnew[price]" type="text" placeholder="0" value="<?php echo $model->price; ?>" /></label>
			<ul class="radio-wrap">
			    <li><label class="radio <?php echo ($model->type_price == 1) ? "checked" : ""; ?>"><input type="radio" value="1" id="onekv" name="radio-price" <?php echo ($model->type_price == 1) ? "checked" : ""; ?> />За м<sup>2</sup></label></li>
			    <li><label class="radio <?php echo ($model->type_price == 2) ? "checked" : ""; ?>"><input type="radio" value="2" id="allkv" name="radio-price" <?php echo ($model->type_price == 2) ? "checked" : ""; ?> />За все</label></li>
			</ul>
		...
		<li>
		    ...
		    <div class="right-part">
			<label class="small-input with-details"><input type="text" id="komprice" placeholder="0" name="addnew[komprice]" value="<?php echo $model->komprice; ?>" /></label>
			<ul class="radio-wrap">
			    <li><label class="radio <?php echo ($model->komprice != 0) ? "checked" : ""; ?>"><input type="radio" value="1" id="yeskom" name="radio-commission" <?php echo ($model->komprice != 0) ? "checked" : ""; ?> />Да</label></li>
			    <li><label class="radio <?php echo ($model->komprice == 0) ? "checked" : ""; ?>"><input type="radio" value="0" id="nokom" name="radio-commission" <?php echo ($model->komprice == 0) ? "checked" : ""; ?> />Нет</label></li>
			</ul>
		    </div>
		</li>
	    </ul>
	...
	<div class="services-and-conditions-wrap">
		...
		<div class="services-and-conditions">
		    <div class="sac_checkbox_list"><?php echo $this->actionGetThreeStepFields($model->functionality, $model->id); ?></div>
		    <div class="clear"></div>
		    <p class="info">Дополнительная информация</p>
		    <span class="symbols" id="addInf_view">120 осталось</span>
		    <textarea class="small" id="addInf_textarea" name="addnew[addInf]" maxlength="120" id="addInf" placeholder="Здесь вы можете уточнить или добавить детали объекта..."><?php echo $model->about; ?></textarea>
...
function ajaxFileUploadListAdd(node, file) {
    ...
    $.post("/items/setparam?p=image&id="+thumb.url+"&array=1");
