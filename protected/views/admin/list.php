<h2>Администрирование объектов <span class="comment">всего <?php echo count($model); ?> объектов</span></h2>
	...
	<?php foreach ($model as $item) { ?>
    	<li class="search-object-item">
			...
			switch ($item->status) {
				...
				break; // DeActive
			    case 3:
				$link[0] = '<a href="#" id="mp-' . $item->id . '" class="moder-play moderation-icon"><i class="loader"></i></a>';
				$link[1] = '<a href="#" id="p-' . $item->id . '" class="pause chosen"><i class="icon-27"></i></a>';
				break; // Moderate
			    case 6:
			...
			?>
    		    ...
    		    <li><a href="<?php echo $this->createUrl('admin/updateitem', array('id' => $item->id)); ?>"><i class="icon-28"></i></a></li>
...
<script type="text/javascript">
    ...
    $(function () {
	var publ_url = "/admin/publitem?id=";
	var paus_url = "/admin/pauseitem?id=";
	$(document).on("click", ".moder-play", function () {
	    data = $(this).attr('id').split("-");

	    if ($(this).hasClass('moderation-icon')) {
		$(this).removeClass('moderation-icon');
		$(this).html('<i class="icon-29"></i>');
