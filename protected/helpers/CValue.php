...
class CValue {
	...
	foreach (array_reverse(str_split(str_pad($num, ceil(strlen($num) / 3) * 3, '0', STR_PAD_LEFT), 3))as $k => $p) {
		...
		if (!$pp)
			...
			case 1:if ($pp == 1) {
				...
				else$o[$k][] = $m[3][$pp];break;
			case 2:if (($k == 1) && ($pp <= 2))
				$o[$k][] = $m[5][$pp];
			    
				else$o[$k][] = $m[1][$pp];break;
		    }$p*=1;
	    if (!$r[$k])
		$r[$k] = reset($r);
	    ...
	    if ($p && $k)
		switch (true) {
		    case preg_match("/^[1]$|^\\d*[0,2-9][1]$/", $p):$o[$k][] = $r[$k][0] . $r[$k][1];
			break;
		    case preg_match("/^[2-4]$|\\d*[0,2-9][2-4]$/", $p):$o[$k][] = $r[$k][0] . $r[$k][2];
			break;
		    default:$o[$k][] = $r[$k][0] . $r[$k][3];
			break;
		}$o[$k] = implode(' ', $o[$k]);
	}
    ...
    static function getValField($item_id) {
	$model = FuncFieldsValues::model()->findAllByAttributes(array('item_id' => $item_id));

	if (isset($model) || (count($model) > 0)) {
	    foreach ($model as $item) {
		$arr[$item->field_id] = $item->value;
	    }
	    return $arr;
	} else {
	    return array();
